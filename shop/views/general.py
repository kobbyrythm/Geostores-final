...
def search(request):
    if request.method == 'GET':
        ...
        movies =  Product.objects.filter(title=search)
        return render(request, 'shop/search.html',{'movies':movies})


    # if request.method== "post":
    #     search= request.post ('search')
    #     movies= Product.objects.filter(name__contains=search)
    #     return render(request, 'shop/search.html',{'search':search,'movies':movies})
    # else:
